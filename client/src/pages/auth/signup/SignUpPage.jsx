	...
	const { mutate: SignUpMutation , isError, isPending, error } = useMutation({
		mutationFn: async ({ email, username, fullName, password }) => {
			try {
				...
				if (!res.ok) throw new Error(data.error || "Failed  creating account");
				console.log(data);
				return data;
			} catch (error) {
				console.error(error);
		...
		<div className='max-w-screen-xl mx-auto flex flex-col sm:flex-row px-10'>
			...
			<div className='flex-1 flex flex-col justify-center items-center'>
				<form className='lg:w-2/3  mx-auto md:mx-20 flex gap-4 flex-col' onSubmit={handleSubmit}>
					...
					<h1 className='text-4xl font-extrabold text-white'>Start your journey</h1>
				...
				<div className='flex flex-col lg:w-2/3 gap-2 mt-4'>
					...
					<Link to='/login'>
						<button className='btn rounded-full border-indigo-800 text-white btn-outline w-full'>Sign in</button>
					</Link>
				</div>
			</div>
		</div>
